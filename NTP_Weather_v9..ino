/*
*  NTP Scrolling Weather ( FC16 + ESP8266 + Piezo hourly chimes )
*/
...
void printLocalTime()
{
 ...
 String Chime  = timex; 
 String Beep   = Chime.substring(6,11);
 if(Beep==("00:00") && bip==0){tone(Pz,f);delay(50);noTone(Pz);delay(50);tone(Pz,f);delay(50);noTone(Pz);bip=1; }
 if(Beep==("30:00") && bip==0){tone(Pz,f);delay(50);noTone(Pz);bip=1;}
 if(Beep==("30:01") || Beep==("00:01")){bip = 0;}
...
{   
 ...
 if (client.connect(servername, 80)) {
  ...
  if( windDir >= 338 ||windDir >= 0 && windDir<23) {Dir = "N" ; Arrow = 178; }
  if( windDir >= 23  && windDir < 68 ){ Dir ="NE" ; Arrow = 179; }
  if( windDir >= 68  && windDir < 113){ Dir ="E"  ; Arrow = 180; }
  if( windDir >= 113 && windDir < 158){ Dir ="SE" ; Arrow = 181; }
  if( windDir >= 158 && windDir < 203){ Dir ="S"  ; Arrow = 182; }
  if( windDir >= 203 && windDir < 248){ Dir ="SW" ; Arrow = 183; }
  if( windDir >= 248 && windDir < 293){ Dir ="W"  ; Arrow = 184; }
  if( windDir >= 293 && windDir < 338){ Dir ="NW" ; Arrow = 185; }
