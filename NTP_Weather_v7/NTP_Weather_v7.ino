...
String APIKEY 		          = "free-api-key";   // get 1 free api key( token) upon account creation ,openweathermap.org
...
{   
 ...
 if (client.connect(servername, 80)) {
  ...
  int    windDeg        = root["wind"]["deg"];    //wind coming from
  int    vision         = root["visibility"];
  int    rssi           = WiFi.RSSI();
  int    Arrow;          // Wind direction font icon
  int    windDir;        // wind direction the opposite of windDeg
  String Dir;            // Wind by Compass North,East..etc
